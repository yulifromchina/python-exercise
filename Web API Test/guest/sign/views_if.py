...
def get_event_list(request):
    ...
    if eid != '':
        ...
        try:
            ...
            event['status'] = result.start_time
    ...
    if name != '':
        ...
        if results:
            for r in results:  # 通过name模糊查询，查询的数据可能有多条，所以用列表存储
                event = {}
                event['name'] = r.name
                event['limit'] = r.limit
...
def user_sign(request):
    ...
    if eid=='' or phone=='':
        return JsonResponse({'status':10001,'message':'paramter error'})
    ...
    if not result:
        return JsonResponse({'status':10012,'message':'user phone null'})

    result = Guest.objects.filter(event_id=eid, phone=phone)
    if not result:
        return JsonResponse({'status':10013,'message':'user id not participate in the conference'})

    result = Guest.objects.get(event_id=eid, phone=phone).sign
    if result:
        return JsonResponse({'status':10014,'message':'user has sign in'})
    else:
